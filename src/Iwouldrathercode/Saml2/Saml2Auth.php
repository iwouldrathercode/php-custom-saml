...
namespace Iwouldrathercode\Saml2;

use OneLogin\Saml2\Utils as OneLogin_Saml2_Utils;
use OneLogin\Saml2\Auth as OneLogin_Saml2_Auth;
use OneLogin\Saml2\Error as OneLogin_Saml2_Error;
use Iwouldrathercode\Saml2\Events\Saml2LogoutEvent;
...
class Saml2Auth
{
    ...
    public static function loadOneLoginAuthFromIpdConfig($idpName)
    {
        ...
        if (isset($config['idp']['x509certMulti'])) {
            if (isset($config['idp']['x509certMulti']['signing'])) {
                foreach ($config['idp']['x509certMulti']['signing'] as $i => $cert) {
                    $config['idp']['x509certMulti']['signing'][$i] = OneLogin_Saml2_Utils::formatCert($cert);
                }
            }
            if (isset($config['idp']['x509certMulti']['encryption'])) {
                foreach ($config['idp']['x509certMulti']['encryption'] as $i => $cert) {
                    $config['idp']['x509certMulti']['encryption'][$i] = OneLogin_Saml2_Utils::formatCert($cert);
                }
            }
        }
